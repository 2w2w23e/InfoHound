...
- If this index is stale, propose an update and ask for approval before relying
  on the new route.
- Wildcard routes such as `roles/*`, `memory/*`, or `decisions/*` mean discover
  candidate paths first, then read only relevant files. Do not silently expand to
  every file unless the task requires a full audit.
...
- relevant files under `user_preferences/`
- relevant files under `memory/`
- relevant files under `decisions/`
- relevant files under `anti_patterns/`

Generic preferences about language, tone, formatting, or answer style should be
routed through the User Preference Update route, not Role Integration, unless
they materially affect role authority or execution behavior.
...
- `PROJECT_STATE.md`
- `PROJECT_INTAKE.md`
- relevant existing files under `roles/`
